...
function buildEmailHtml(firstName: string, results: AIResults, topGifts: GiftName[], allScores: GiftScores): string {
  const sortedScores = (Object.entries(allScores) as [GiftName, number][])
    .sort((a, b) => b[1] - a[1]);
  ...
  const barChart = sortedScores.map(([gift, score], i) => {
    ...
    const color = isTop3 ? '#34C6F4' : '#333';
    return `
      <tr>
        <td style="font-size:12px;color:${isTop3 ? '#000' : '#888'};font-weight:${isTop3 ? 'bold' : 'normal'};width:110px;padding:2px 6px 2px 0;">${gift}</td>
        <td style="padding:2px 0;">
          <div style="background:#eee;border-radius:4px;height:10px;width:200px;">
            <div style="background:${color};border-radius:4px;height:10px;width:${pct * 2}px;"></div>
          </div>
        </td>
        <td style="font-size:11px;color:${isTop3 ? '#34C6F4' : '#aaa'};padding:2px 0 2px 6px;">${score}</td>
      </tr>`;
  }).join('');

  const actionPlanHtml = results.actionPlan?.map(week => `
    <p style="margin:16px 0 4px;"><strong>Week ${week.week}: ${week.theme}</strong></p>
    <ul style="margin:0;padding-left:20px;color:#333;">
      ${week.actions.map(a => `<li style="margin-bottom:4px;">${a}</li>`).join('')}
    </ul>
  `).join('') || '';
...
<body style="font-family:Georgia,serif;max-width:600px;margin:0 auto;padding:20px;background:#fff;color:#111;">
  ...
  ${results.topGifts.map((g, i) => `
    <p style="margin:0 0 4px;"><strong>${i + 1}. ${g.name}</strong></p>
    <p style="color:#333;line-height:1.6;margin:0 0 16px;">${g.description}</p>
  `).join('')}

  ${results.giftSynergy ? `
  <div style="background:#f0f9ff;border-left:3px solid #34C6F4;padding:12px 16px;margin:0 0 24px;border-radius:4px;">
    <p style="color:#333;font-style:italic;margin:0;">${results.giftSynergy}</p>
  </div>` : ''}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (email) {
      const html = buildEmailHtml(firstName, results, topGifts, combined);
      await sendEmail(email, firstName, html);
    }
